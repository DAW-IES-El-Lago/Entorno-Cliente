...
class Video{
	constructor(ns,a,b,c,d,e,f) {
		...
		this.vistas = (function(){ // closure permite atributos privados y estaticos
			var contador=f; 
			return function (){contador+=1;return contador}
		})();
	}
	get getHTML() {	
		var img = '<a href="#openModal"><img src="https://i.ytimg.com/vi/'+
			this._vid+'/mqdefault.jpg" alt="foto"/></a>';
		var txt = '<div class="titulo">'+this._titulo+'</div>'+
			this._t1+'<br/>'+this._subida.toLocaleDateString()+'<br/><div class="visitas">'+
			this._visitas+'</div>'+this._ns+'(<span id="'+this._vid+'vistas">'+this.vistas()+'</span>)';
	...
	get getVistas() { return this.vistas(); }
	set ns(x) { this._ns=x; }
	set vid(x) { this._vid=x; }
	set titulo(x) { this._titulo=x; }
	set duracion(x) { this._duracion=x; }
	set subida(x) { this._subida=x; }
	set visitas(x) { this._visitas; }
